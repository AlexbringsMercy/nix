...
- Four-finger gestures dead; pinch-to-zoom inconsistent (see 4.8).
...
- **agridyne/dotfiles-dt** — TIER 1 for cohesion: sidebar, cohesive glass widgets, glass app icons/launchers (the YouTube tile), login screen, sidebar visualizer sizing, whole-system monochrome-glass consistency. KDE is NOT a disqualifier — it's a code check + compat pass, like any framework difference. FULL repo read required.
- **caelestia-dots/caelestia** — the MAIN repo, not just /shell: launcher (click-away via HyprlandFocusGrab), sidebar (3-module composite), hover popout top bar with music + source-app icon (Spotify) that opens the themed app, runtime-configurable opacity.
- **snowarch/iNiR** — aurora/angel dark-glass style presets, live glass tuner, palette/token architecture, motion curves, font sizing. Niri-first: ADAPT for Hyprland (compat conversion), never rebuild from scratch.
...
- **hellwal** — palette-generation alternative if Matugen stays problematic.
...
- **DankMaterialShell** — Modules/Dock (≈4 files): left-vertical dock, pinned + running grouped icons.
...
- Matugen 4.0 silently discards the custom_colors teal bias key (verify against installed version; explains palette drift)
