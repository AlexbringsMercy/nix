...
**subordinate to `GRAND_PLAN.md`** and can never override it. Where anything
below reads as architecture, surface ownership, or palette authority, and it
conflicts with `GRAND_PLAN.md`, **the Grand Plan wins, always** — this file is
...
`archive/superseded-docs/2026-07-29/visual-design-reference.md`. This
replacement was written on 2026-07-29 after `GRAND_PLAN.md` was revised
...
(`GRAND_PLAN.md` §3.1, §4.0, decisions 15–16 in §10.2) is:
...
`GRAND_PLAN.md` §3.1 and §4.0. Do not restate hex values here; they are
...
`GRAND_PLAN.md` §3.2 gives dark-auto and light-auto starting ranges per
...
this document first and are now the literal source for `GRAND_PLAN.md` §3.3's
...
`reducedMotion`) in `GRAND_PLAN.md` §3.3, one token table owned by the theme
...
of visual scale. All of this matches `GRAND_PLAN.md` §3.4 as written.
...
|---|---|---|
| "Apps sidebar or dock, second UI surface" | The left application rail: launcher, pinned apps, current-workspace running/minimized windows, grouped previews, agridyne glass-tile treatment on the app buttons | `GRAND_PLAN.md` §5.1, §5.3 |
...
mandate — see `GRAND_PLAN.md` §2.1 and §13 for the authoritative table.
...
Waybar itself; kept only as historical references per `GRAND_PLAN.md` §13.2.
...
|---|---|---|
| Fixed hex palette (`#0a0e1a`–`#131729`, teal/purple accents as *the* system) | **Dropped** — was a fixed dark identity; contradicts wallpaper-derived, mode-switching palette | `GRAND_PLAN.md` §3.1, §4.0 |
...
| Functional/input bug list (disable_while_typing, scroll sensitivity, border resize, tiling bug, Cmd+Space, drag-by-titlebar, backspace acceleration, Cmd+arrow snapping, Ctrl+T, weather location, CPU/Memory button dupe, EasyEffects visibility, wallpaper-picker location, screenshot overlay) | **Migrated in full** — these are functional/input bugs, not visual design, so this document (now scoped to visual intent) doesn't restate them | `ISSUE_LOG.md`; traced item-by-item in `GRAND_PLAN.md` §15 |
| "Search 3-5 community implementations, view previews before ranking" protocol | **Migrated** — this is now a standing execution rule, not visual-doc content | `SESSION_PREAMBLE.md`, `PM_OPERATING_RULES.md` |
...
- The original doc's Tier-1 framing put **agridyne** and **caelestia-dots/
  ...
  reference. Current authority (`GRAND_PLAN.md` §2.1) is more surgical:
...
- `GRAND_PLAN.md` — design and architecture authority; this document is
  subordinate to it on every point above.
- `MASTER_REQUIREMENTS.md` — the requirements ledger.
- `ISSUE_LOG.md` — the functional bug register (see migration table above).
- `SOURCES.md` — community provenance ledger.
- `docs/controls.md` — current keybinds and mouse paths.
- `docs/recovery.md` — rollback procedures.
- `docs/wallpapers.md` — wallpaper library/import notes.
- `archive/superseded-docs/2026-07-29/visual-design-reference.md` — the
