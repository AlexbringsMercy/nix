...
5. **The window model is mouse-first and workspace-coherent.** Hyprbars provides ordinary controls; keyboard focus changes on click while scrolling follows the pointer; exact left/right two-pane snap **reserves** a half and reflows the rest, minimizing only when both halves are reserved by half-snap owners (§6.2); minimize never creates a user-visible or conceptual workspace.
...
5. Super+Left/Right creates exact two-pane work by **reserving** a half for the active window; ordinary windows reflow in the other half and stay visible. Only when **both halves are reserved by explicit half-snap owners** do the remaining windows minimize into that same workspace's rail. Releasing a reservation returns that half to ordinary tiling. (Full reservation/reflow model — not a pair model — in §6.2.)
...
- **Deterministic two-pane snap — reservation / reflow model (clarified 2026-08-09; this is NOT a pair model):** `Super+Left` always makes the active window the exact left 50%; `Super+Right` the exact right 50%, independent of dwindle tree shape or number of windows. A half-snap makes that window an **explicit half-snap owner** — a reservation with higher layout priority that occupies the **entire** usable half. **At most one owner per half.** Ordinary (un-snapped) tiled windows are a **reflow pool**:
    - If only **one** half is reserved, every other eligible ordinary window **stays visible** and reflows/tiles inside the opposite unreserved half. Ordinary windows are **not** minimized merely because one half is occupied by an owner.
    - Windows minimize **only when both halves are reserved by explicit half-snap owners and no ordinary tiled region remains.** Wherever this spec says a half is "occupied" or "both halves occupied," it means **reserved by half-snap owners** — never ordinary windows merely existing in both halves.
    - Snap onto an already-reserved half: the new target becomes owner; the previous owner is **demoted to ordinary** and joins the opposite half's reflow pool (or overflow/minimized if that half is also reserved).
    - Moving an owner across sides: release the origin reservation, claim the destination, demote any prior destination owner; ordinary windows reflow into the newly unreserved origin half.
    - Worked examples: **three** windows, snap one left → left reserved, the other two reflow in the right half, **neither minimized**. **Four** windows, snap one left then another right → both halves reserved → remaining ordinary windows minimize. Do **not** default to MRU pair behavior.
- **Reservation release / restoration:** releasing a reservation — dragging/unsnapping/maximizing an owner, closing an owner, or restoring a minimized window when a half frees up — returns that region to the ordinary reflow pool and re-tiles normally; prior placement/state is restored where technically possible. A topology snapshot may aid reflow continuity, but exact old geometry is not sacred when the available region has changed. Clicking a restored window is expected to bring an additional window into view.
- **Snap gate:** one, two, and 3+ windows; target side already reserved; left/right; second-press return; single-owner reflow (other windows stay visible, not minimized); both-halves-reserved minimize; owner demotion on cross-side snap; restore; drag; close; maximize; new window while a half is reserved.
...
| M27 | Deterministic two-pane snap orchestration (reservation/reflow, §6.2), surplus minimization, reservation release/restoration | M | §6.2 |
...
and acceptance conditions, remains in `docs/reports/EXECUTION_LOG.md`; these are the binding
...
Record in `docs/reports/EXECUTION_LOG.md` with the active generation and operator-decision numbers:
