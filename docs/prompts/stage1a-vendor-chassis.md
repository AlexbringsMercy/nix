...
Copy the full tree `/home/alex/nix/repos/caelestia/shell-main/shell-main/` → `/home/alex/nix/modules/home/aurora-shell/` — everything: `shell.qml`, `plugin/`, `services/`, `modules/`, `components/`, `utils/`, `scripts/`, `extras/`, `assets/`, `nix/`, `CMakeLists.txt`, `flake.nix`, `flake.lock`, `LICENSE`, `README.md`. Exclude only a `.git` directory if one exists. This exact snapshot is the audited source of truth (inventoried in `docs/research/caelestia-full-inventory.md`) — do not substitute or refresh anything.
