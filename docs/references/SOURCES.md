...
Reorganized to mirror `docs/plans/GRAND_PLAN.md` §13's structure so this ledger and the plan's own source
...
| luisbocanegra/kurve (web only, via agridyne's screenshots + `docs/research/agridyne.md`) | not applicable — reference only | Optional left-rail cava-strip visualizer: sizing/behavior reference (orientation, block width/gap, transparent bg). A KDE Plasma plasmoid, not QuickShell-native — requires reimplementation over the existing shared `cava` provider, not a code port. |
...
picker itself stays on-theme). See `docs/plans/GRAND_PLAN.md` §4.0 for the full ownership-chain diagram.
...
| ekremx25 (web only, not cloned — consistent with `docs/prompts/SESSION_PREAMBLE.md`'s web-only list) | not applicable | Display-panel row-layout visual reference; DMS remains the functional Display backend |
...
- **UNRESOLVED, needing the operator or PM, not guessed at:** agridyne's lock-visual-composition
  ...
  (active row in `docs/references/SOURCES.md`, absent from the revised plan, no explicit rejection found —
...
**Read in full:** `docs/prompts/SESSION_PREAMBLE.md`; `docs/plans/GRAND_PLAN.md` (all 940 lines, not just §13/§2.1 as
strictly required — needed the whole document to know which body-text sections corroborate or
contradict the source map); `docs/references/SOURCES.md` (78 lines); `docs/reports/EXECUTION_LOG.md` (all 1918 lines, plus
the header banner in detail); `docs/plans/MASTER_REQUIREMENTS.md` (350 lines); `docs/plans/STAGE2_CLOSEOUT_WORK_ORDER.md`
...
README.md` in full; `docs/instructions/wallpapers.md`, `docs/instructions/controls.md`, `docs/instructions/recovery.md` in full;
`docs/prompts/stage2b-keymap-osd-input.md` in full; the full `git diff` of `docs/plans/GRAND_PLAN.md`
against `HEAD` (1030 lines) to establish exactly what the 2026-07-29 revision changed versus
what was already true.

**Verified on disk (directory/file listings, not just grep):** every repo named in `docs/plans/GRAND_PLAN.md`
...
for every one of them (§4/proposed table) rather than assuming the pins already in `docs/references/SOURCES.md`
were still current.
...
web-only in `docs/prompts/SESSION_PREAMBLE.md`. Did not open every file inside every verified repo (e.g., I
...
superseded-where-conflicting per `docs/plans/GRAND_PLAN.md`'s own header, so I treated them as lower
priority than the five documents the task named directly. Did not read `docs/reports/CURRENT_STATE_AUDIT.md`
or `docs/instructions/PM_OPERATING_RULES.md` (referenced by `docs/plans/STAGE2_CLOSEOUT_WORK_ORDER.md` but not named in this
task's scope). Did not inspect `docs/archive/superseded-handoffs/2026-07-28/` beyond the one grep hit
